...
def get_config_value(config, option_name):
    ...
    if cmd_value is not None:
        # Handle list values (if returned as a list, extract first element)
    ...
    if option_name == "headed":
        ...
        ini_value = config.getini(option_name)
        # Handle list values from ini file
        if isinstance(ini_value, list) and len(ini_value) > 0:
            return ini_value[0]
        return ini_value
